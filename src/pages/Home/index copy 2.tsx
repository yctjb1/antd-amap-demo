          ...
          if (viewerRef.current && viewerRef.current.cesiumElement) {
            ...
            viewer.screenSpaceEventHandler.setInputAction((movement) => {
              ...
              if (Cesium.defined(pickedObject)) {
                ...
                const clickedNode = pickedObject.detail?.node
                // 使用更标准的方式获取节点名称，并尝试通过 getNode 验证
                const nodeName = clickedNode?._node?.name || clickedNode?._name
                if (model && clickedNode && nodeName && nodeName !== 'root') {
                  console.log('点击到具体节点：', {
                    name: nodeName,
                    index: clickedNode._node.index,
                  })

                  // 使用 getNode 方法验证节点存在性
                  if (typeof model.getNode === 'function') {
                    try {
                      const validatedNode = model.getNode(nodeName)
                      if (validatedNode) {
                        console.log('节点验证成功')
                        // 使用验证后的节点
                        onNodeClick?.(pickedObject.id, validatedNode, nodeName)
                      } else {
                        console.warn('节点验证失败：模型中不存在该节点')
                        // 回退到原始节点
                        onNodeClick?.(pickedObject.id, clickedNode, nodeName)
                      }
                    } catch (e) {
                      console.warn('节点验证出错，使用原始节点：', e)
                      // 出错时回退到原始节点
                      onNodeClick?.(pickedObject.id, clickedNode, nodeName)
                    }
                  } else {
                    // 模型不支持 getNode，使用原始节点
                    onNodeClick?.(pickedObject.id, clickedNode, nodeName)
                  }
...
const EntityInfoPanel = ({
  ...
  }, [entity, nodeData, allEntityData, form])
...
const Home = () => {
  ...
  const originalNodeColorsRef = useRef<Map<any, any>>(new Map())
  const { viewer } = useCesiumViewer()
  const isProcessingRef = useRef(false) // 用于防止重复处理的锁
  ...
  const highlightNode = useCallback(
    (
      ...
      // 检查是否是通过 getNode 获取的正规节点
      if (node._model && node._runtimeNode) {
        // 这是通过 getNode 获取的正规 ModelNode
        console.log('使用正规 ModelNode:', node)

        const primitives = node._runtimeNode.runtimePrimitives
        if (!primitives) {
          console.log('节点没有运行时图元，无法修改材质')
          return
        }
        ...
        requestAnimationFrame(() => {
          if (highlight) {
            // 首次高亮时保存原始材质
            if (!originalNodeColorsRef.current.has(node)) {
              const originalMaterials = primitives.map((p: any) => {
                if (p.drawCommand) {
                  return {
                    color: p.drawCommand.color
                      ? Cesium.Color.clone(p.drawCommand.color)
                      : null,
                  }
                }
                return null
              })
              originalNodeColorsRef.current.set(node, originalMaterials)
              console.log('保存节点原始材质信息')

              // 修改每个图元的颜色
              primitives.forEach((primitive: any) => {
                if (primitive.drawCommand) {
                  primitive.drawCommand.color = highlightColor[type]
                  console.log('设置图元高亮颜色')
                }
              })
            }
          } else {
            // 恢复原始材质
            const originalMaterials = originalNodeColorsRef.current.get(node)
            if (originalMaterials) {
              primitives.forEach((primitive: any, index: number) => {
                if (primitive.drawCommand && originalMaterials[index]) {
                  if (originalMaterials[index].color) {
                    primitive.drawCommand.color = Cesium.Color.clone(
                      originalMaterials[index].color
                    )
                    console.log('恢复图元原始颜色')
                  }
                }
              })
              originalNodeColorsRef.current.delete(node)
            }
          }
          ...
          if (viewer && viewer.scene) {
            ...
            console.log('请求场景重新渲染')
          }
        })
      } else {
        // 处理旧的节点结构（向后兼容）
        const primitives = node?._node?.primitives
        if (!node || !primitives) {
          console.log('节点或图元不存在，无法修改材质')
          return
        }
        ...
        requestAnimationFrame(() => {
          if (highlight) {
            // 首次高亮时保存原始材质
            if (!originalNodeColorsRef.current.has(node)) {
              const originalMaterials = primitives.map((p: any) => p.material)
              originalNodeColorsRef.current.set(node, originalMaterials)
              console.log('保存节点原始材质')

              // 为每个图元创建新的颜色材质
              primitives.forEach((primitive: any) => {
                const newMaterial = Cesium.Material.fromType('Color', {
            ...
            if (originalMaterials) {
              primitives.forEach((primitive: any, index: number) => {
                primitive.material = originalMaterials[index]
                console.log('恢复图元原始材质')
              })
              originalNodeColorsRef.current.delete(node)
            }
          }
  ...
  const handleNodeClick = useCallback(
    (entity: any, node: any, nodeName: string) => {
      // 如果当前有选中的实体，先取消实体的高亮
      if (selectedEntity) {
      ...
      setSelectedNode((prevSelectedNode) => {
        const isSameNode = prevSelectedNode?.nodeName === nodeName;

        // 如果点击的是已选中的节点，则取消选择
        if (prevSelectedNode && isSameNode) {
          // 取消高亮
          highlightNode(prevSelectedNode.node, false)
          // 关闭面板
          setShowInfoPanel(false)
          return null
        }
        ...
        // 高亮新选中的节点
        const nodeStatus = nodeData.status || ModelStatus.DEFAULT
        highlightNode(node, true, nodeStatus)
    ...
    [highlightEntity, highlightNode, selectedEntity]
  )
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      <EntityInfoPanel
        ...
        node={selectedNode ? selectedNode.node : null}
        nodeName={selectedNode ? selectedNode.nodeName : null}
        nodeData={selectedNode ? selectedNode.nodeData : null}
        ...
        onClose={() => {
          ...
          if (selectedNode) {
            highlightNode(selectedNode.node, false)
          }
          setSelectedEntity(null)
