        ...
        const timer = setTimeout(() => {
          if (viewerRef.current && viewerRef.current.cesiumElement) {
            ...
            handler.setInputAction((movement: any) => {
              ...
              console.log('pillar name:', pickedObject.getProperty('name'))
              console.log(pickedObject instanceof Cesium.Cesium3DTileFeature)
              ...
              if (Cesium.defined(pickedObject) && pickedObject.primitive) {
                ...
                // 获取tileset的ID（如果已设置）
                const tilesetId = tileset.id || 'unknown-tileset'
      ...
      if (tileset) {
        ...
        switch (colorMode) {
          case 'original':
            ...
            console.log(`恢复3D Tiles ${entityId}原始样式`)
            break
          case 'red':
            // 高亮特定构件为红色，其他保持原样
            tileset.style = new Cesium.Cesium3DTileStyle({
              color: {
                conditions: [
                  ["${name} === 'pillar1'", "color('red')"],
                  [true, "color('#ffffff')"],
                ],
              },
            })
            console.log(`设置3D Tiles ${entityId}中pillar1为红色`)
            break
          case 'green':
            tileset.style = new Cesium.Cesium3DTileStyle({
              color: "color('green')",
            })
            console.log(`设置3D Tiles ${entityId}为绿色`)
            break
          case 'blue':
            tileset.style = new Cesium.Cesium3DTileStyle({
              color: "color('blue')",
            })
            console.log(`设置3D Tiles ${entityId}为蓝色`)
            break
        }
      } else {
        // 处理普通模型（如果有的话）
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      >
        ...
        <Button
          style={{ margin: 5 }}
          onClick={() => testModelColor('test-bridge-model1', 'blue')}
        >
          模型变蓝
        </Button>
      </div>
